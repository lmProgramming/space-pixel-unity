    ...
    internal class HotReloadRunTab : HotReloadTabBase {
        ...
        static void RenderEntries(TimelineType timelineType) {
            ...
            for (var i = 0; i < alertEntries.Count; i++) {
                ...
                var title = $" {alertEntry.title}{(!string.IsNullOrEmpty(alertEntry.shortDescription) ? $": {alertEntry.shortDescription}": "")}";
                Texture2D icon = null;
                GUIStyle style;
                ...
                var clickableDescription = alertEntry.title == "Unsupported change" || alertEntry.title == "Compile error" || alertEntry.title == "Failed applying patch to method";
        ...
        private void RenderRecompileButton() {
            var recompileText = HotReloadWindowStyles.windowScreenWidth > Constants.RecompileButtonTextHideWidth ? " Recompile" : "";
        ...
        private void RenderIndicationButtons() {
            ...
            if (!currentState.running && (currentState.startupProgress?.Item1 ?? 0) == 0) {
                var startText = HotReloadWindowStyles.windowScreenWidth > Constants.StartButtonTextHideWidth ? " Start" : "";
                ...
                var stopText = HotReloadWindowStyles.windowScreenWidth > Constants.StartButtonTextHideWidth ? " Stop" : "";
