...
namespace FastScriptReload.Scripts.Editor.Compilation
{
    public class DynamicAssemblyCompiler
    {
        ...
        var result = McsExeDynamicCompilation.Compile(filePathsWithSourceCode);
            ...
            LoggerScoped.Log(
                $"Files: {string.Join(",", filePathsWithSourceCode.Select(fn => new FileInfo(fn).Name))} changed " +
    ...
    {
        public CompileResult(string compiledAssemblyPath, List<string> messagesFromCompilerProcess,
            int nativeCompilerReturnValue, Assembly compiledAssembly, string sourceCodeCombined,
            string sourceCodeCombinedFileLocation)
        {
