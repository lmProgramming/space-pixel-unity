...
namespace FastScriptReload.Editor.Compilation.CodeRewriting
{
    public class ManualUserDefinedScriptOverridesRewriter : FastScriptReloadCodeRewriterBase
    {
        ...
        public ManualUserDefinedScriptOverridesRewriter(SyntaxNode userDefinedOverridesRoot,
            bool writeRewriteReasonAsComment, bool visitIntoStructuredTrivia = false)
        ...
        public override SyntaxNode VisitConversionOperatorDeclaration(ConversionOperatorDeclarationSyntax node)
        {
            ...
            var matchingInOverride = _userDefinedOverridesRoot.DescendantNodes()
                ...
                .FirstOrDefault(m =>
                        m.ParameterList.ToString() ==
                        node.ParameterList
                            .ToString() //parameter lists is type / order / names, all good for targetting if there's a proper match
                        && methodFqdn ==
                        RoslynUtils.GetMemberFqdn(m,
                            "operator") //make sure same FQDN, even though there's no name there could be more implicit operators in file
                );

            if (matchingInOverride != null)
                return AddRewriteCommentIfNeeded(matchingInOverride.WithTriviaFrom(node),
                    "User defined custom conversion override", true);
        ...
        public override SyntaxNode VisitMethodDeclaration(MethodDeclarationSyntax node)
        {
            ...
            var matchingInOverride = _userDefinedOverridesRoot.DescendantNodes()
                ...
                .FirstOrDefault(m => m.Identifier.ValueText == methodName
                                     ...
                                     node.TypeParameterList?.ToString() //typed paratemets are for generics, also check
                                     && methodFqdn ==
                                     RoslynUtils.GetMemberFqdn(m,
                                         m.Identifier
                                             .ToString()) //last check for mathod FQDN (potentially slower than others)
                );

            if (matchingInOverride != null)
                return AddRewriteCommentIfNeeded(matchingInOverride.WithTriviaFrom(node),
                    "User defined custom method override", true);
                ...
                .FirstOrDefault(d =>
                    {
                        ...
                        return declarationName == name
                               ...
                               && fqdn == RoslynUtils.GetMemberFqdn(d,
                                   declarationName); //last check for mathod FQDN (potentially slower than others)
                    }
                );
