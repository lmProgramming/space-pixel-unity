    ...
    internal class NewFieldsRewriter : FastScriptReloadCodeRewriterBase
    {
        ...
        public static List<MemberInfo> GetReplaceableMembers(Type type)
        {
            ...
            return type.GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
                                  BindingFlags.NonPublic).Cast<MemberInfo>().ToList();
        }
        ...
        public override SyntaxNode VisitInvocationExpression(InvocationExpressionSyntax node)
        {
            if (node.Expression.ToString() == "nameof")
            {
                ...
                if (classNode != null)
                {
                    ...
                    {
                        var nameofExpressionParts =
                            node.ArgumentList.Arguments.First().ToFullString()
                                .Split('.'); //nameof could have multiple . like NewFieldCustomClass.FieldInThatClass
                        var fieldName =
                            nameofExpressionParts
                                .First(); // should take first part only to determine if new field eg. 'NewFieldCustomClass'
                            ...
                            if (allNewFieldNamesForClass.Contains(fieldName))
                                return AddRewriteCommentIfNeeded(
                                    SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression,
                                        SyntaxFactory.Literal(nameofExpressionParts
                                            .Last())), // should take last part only to for actual string eg. 'FieldInThatClass'
            ...
            if (classNode != null)
            {
                ...
                if (!string.IsNullOrEmpty(fullClassName))
                {
                    ...
                    if (_typeToNewFieldDeclarations.TryGetValue(fullClassName, out var allNewFieldNamesForClass))
                    {
                        if (allNewFieldNamesForClass.Contains(fieldName))
                        {
                            var isNameOfExpression = node.Ancestors().OfType<InvocationExpressionSyntax>()
                                .Any(e => e.Expression.ToString() == "nameof");
                                    ...
                                    AddRewriteCommentIfNeeded(
                                        SyntaxFactory.MemberAccessExpression(
                                                ...
                                                SyntaxFactory.InvocationExpression(
                                                        SyntaxFactory.MemberAccessExpression(
                                                            SyntaxKind.SimpleMemberAccessExpression,
                                                            SyntaxFactory.IdentifierName(typeof(TemporaryNewFieldValues)
                                                                .FullName),
                                                            SyntaxFactory.GenericName(
                                                                    SyntaxFactory.Identifier(
                                                                        nameof(TemporaryNewFieldValues
                                                                            .ResolvePatchedObject)))
                ...
                if (newFields.Contains(fieldName))
                {
                    ...
                    return AddRewriteCommentIfNeeded(
                        node
                            ...
                            .WithTrailingTrivia(existingTrailing.Insert(0,
                                SyntaxFactory.Comment(" */ //Auto-excluded to prevent exceptions - see docs"))),
