    ...
    internal class ConstructorRewriter : FastScriptReloadCodeRewriterBase
    {
        ...
        public override SyntaxNode VisitConstructorDeclaration(ConstructorDeclarationSyntax node)
        {
            if (_adjustCtorOnlyForNonNestedTypes)
            {
                ...
                if (typeNestedLevel == 1) return AdjustCtorOrDestructorNameForTypeAdjustment(node, node.Identifier);
            }
            else
            {
        ...
        private SyntaxNode AdjustCtorOrDestructorNameForTypeAdjustment(BaseMethodDeclarationSyntax node,
            SyntaxToken nodeIdentifier)
        {
