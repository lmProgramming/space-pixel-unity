...
namespace FastScriptReload.Scripts.Editor.Compilation.CodeRewriting
{
    internal abstract class ThisRewriterBase : FastScriptReloadCodeRewriterBase
    {
        ...
        {
            var ancestors = node.Ancestors().Where(n => n is TypeDeclarationSyntax).Cast<TypeDeclarationSyntax>()
                .ToList();
            if (ancestors.Count() > 1)
            {
                LoggerScoped.LogWarning(
                    $"ThisRewriter: for class: '{ancestors.First().Identifier}' - 'this' call/assignment in nested class / struct. Dynamic cast will be used but this could cause issues in some cases:" +
            ...
            {
                LoggerScoped.LogWarning(
                    $"Unable to find first ancestor for node: {node.ToFullString()}, this rewrite will not be applied");
                return node;
            }
            ...
            return AddRewriteCommentIfNeeded(resultNode,
                $"{nameof(ThisRewriterBase)}:{nameof(CreateCastedThisExpression)}");
        }
    }
}