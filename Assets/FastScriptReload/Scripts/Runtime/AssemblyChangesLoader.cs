...
namespace FastScriptReload.Runtime
{
    ...
    public class AssemblyChangesLoader : IAssemblyChangesLoader
    {
        ...
        private const BindingFlags AllDeclaredMethodsBindingFlags = BindingFlags.Public | BindingFlags.NonPublic |
                                                                    BindingFlags.Static | BindingFlags.Instance |
                                                                    BindingFlags
                                                                        .DeclaredOnly; //only declared methods can be redirected, otherwise it'll result in hang

        public const string ClassnamePatchedPostfix = "__Patched_";
        public const string OnHotReloadMethodName = "OnScriptHotReload";
        public const string OnHotReloadNoInstanceStaticMethodName = "OnScriptHotReloadNoInstance";

        private static readonly List<Type> ExcludeMethodsDefinedOnTypes = new()
        {
        ...
        private readonly Dictionary<Type, Type> _existingTypeToRedirectedType = new();
        ...
        public void DynamicallyUpdateMethodsForCreatedAssembly(Assembly dynamicallyLoadedAssemblyWithUpdates,
            AssemblyChangesLoaderEditorOptionsNeededInBuild editorOptions)
        {
            try
            {
                        ...
                        )
                {
                    if (createdType.GetCustomAttribute<PreventHotReload>() != null)
                    {
                        ...
                        LoggerScoped.Log(
                            $"Type: {createdType.Name} marked as {nameof(PreventHotReload)} - ignoring change.");
                        continue;
                    }
                    ...
                    if (ProjectTypeCache.AllTypesInNonDynamicGeneratedAssemblies.TryGetValue(
                            createdTypeNameWithoutPatchedPostfix, out var matchingTypeInExistingAssemblies))
                    {
                        ...
                        var allDeclaredMethodsInExistingType = matchingTypeInExistingAssemblies
                            .GetMethods(AllDeclaredMethodsBindingFlags)
                            .Where(m => !ExcludeMethodsDefinedOnTypes.Contains(m.DeclaringType))
                            .ToList();
                        foreach (var createdTypeMethodToUpdate in createdType.GetMethods(AllDeclaredMethodsBindingFlags)
                                     .Where(m => !ExcludeMethodsDefinedOnTypes.Contains(m.DeclaringType)))
                        {
                            var createdTypeMethodToUpdateFullDescriptionWithoutPatchedClassPostfix =
                                RemoveClassPostfix(createdTypeMethodToUpdate.FullDescription());
                            var matchingMethodInExistingType = allDeclaredMethodsInExistingType
                                .SingleOrDefault(m =>
                                    m.FullDescription() ==
                                    createdTypeMethodToUpdateFullDescriptionWithoutPatchedClassPostfix);
                            if (matchingMethodInExistingType != null)
                            {
                                if (matchingMethodInExistingType.IsGenericMethod)
                                {
                                    LoggerScoped.LogWarning(
                                        $"Method: '{matchingMethodInExistingType.FullDescription()}' is generic. Hot-Reload for generic methods is not supported yet, you won't see changes for that method.");
                                    continue;
                                }

                                if (matchingMethodInExistingType.DeclaringType != null &&
                                    matchingMethodInExistingType.DeclaringType.IsGenericType)
                                {
                                    LoggerScoped.LogWarning(
                                        $"Type for method: '{matchingMethodInExistingType.FullDescription()}' is generic. Hot-Reload for generic types is not supported yet, you won't see changes for that type.");
                                    continue;
                                }

                                LoggerScoped.LogDebug(
                                    $"Trying to detour method, from: '{matchingMethodInExistingType.FullDescription()}' to: '{createdTypeMethodToUpdate.FullDescription()}'");
                            ...
                            {
                                LoggerScoped.LogWarning(
                                    $"Method: {createdTypeMethodToUpdate.FullDescription()} does not exist in initially compiled type: {matchingTypeInExistingAssemblies.FullName}. " +
                    ...
                    {
                        LoggerScoped.LogWarning(
                            $"FSR: Unable to find existing type for: '{createdType.FullName}', this is not an issue if you added new type. <color=orange>If it's an existing type please do a full domain-reload - one of optimisations is to cache existing types for later lookup on first call.</color>");
                        FindAndExecuteStaticOnScriptHotReloadNoInstance(createdType);
        ...
        {
            var createdTypeFieldAndProperties = createdType.GetFields(AllBindingFlags)
                .Concat(createdType.GetProperties(AllBindingFlags).Cast<MemberInfo>()).ToList();
            var matchingTypeFieldAndProperties = matchingTypeInExistingAssemblies.GetFields(AllBindingFlags)
                .Concat(matchingTypeInExistingAssemblies.GetProperties(AllBindingFlags).Cast<MemberInfo>()).ToList();
            if (createdTypeFieldAndProperties.Count != matchingTypeFieldAndProperties.Count)
            {
                var addedMemberNames = createdTypeFieldAndProperties.Select(m => m.Name)
                    .Except(matchingTypeFieldAndProperties.Select(m => m.Name)).ToList();
                LoggerScoped.LogError(
                    $"It seems you've added/removed field to changed script. This is not supported and will result in undefined behaviour. Hot-reload will not be performed for type: {matchingTypeInExistingAssemblies.Name}" +
        ...
        {
            var onScriptHotReloadStaticFnForType = createdType.GetMethod(OnHotReloadNoInstanceStaticMethodName,
        ...
        {
            var onScriptHotReloadFnForType = originalType.GetMethod(OnHotReloadMethodName,
            ...
            else
            {
                ...
                var onScriptHotReloadFnForCreatedType = detourType.GetMethod(OnHotReloadMethodName,
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (onScriptHotReloadFnForCreatedType != null)
                {
                    ...
                    {
                        LoggerScoped.LogWarning(
                            $"Unable to find DynamicMethod, added {OnHotReloadMethodName} won't be called. Make sure to add method before initial compilation.");
                        return;
                    }

                    var dynamicMethodCtor =
                        dynamicMethodType.GetConstructor(new[] { typeof(string), typeof(Type), typeof(Type[]) });
                    var dynamicMethodDynamicallyAdded = (MethodInfo)dynamicMethodCtor.Invoke(new object[]
                        { OnHotReloadMethodName + "_DynamicallyAdded", typeof(void), new Type[] { } });
        ...
        private static void ExecuteFnOnMainThread(Type originalType, MethodInfo onScriptHotReloadFn)
        {
            UnityMainThreadDispatcher.Instance.Enqueue(() =>
            {
                ...
                {
                    LoggerScoped.LogWarning(
                        $"Type: {originalType.Name} is not {nameof(MonoBehaviour)}, {OnHotReloadMethodName} method can't be executed. You can still use static version: {OnHotReloadNoInstanceStaticMethodName}");
                    return;
                }

                foreach (var instanceOfType in
                         GameObject.FindObjectsOfType(originalType)) //TODO: perf - could find them in different way?
...
#pragma warning restore 0618
        ...
        public void UpdateValues(bool isDidFieldsOrPropertyCountChangedCheckDisabled,
            bool enableExperimentalAddedFieldsSupport)
        {
            this.isDidFieldsOrPropertyCountChangedCheckDisabled = isDidFieldsOrPropertyCountChangedCheckDisabled;
            this.enableExperimentalAddedFieldsSupport = enableExperimentalAddedFieldsSupport;
