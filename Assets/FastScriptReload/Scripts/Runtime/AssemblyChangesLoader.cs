...
namespace FastScriptReload.Scripts.Runtime
{
    ...
    public class AssemblyChangesLoader : IAssemblyChangesLoader
    {
                        ...
                        )
                {
                            ...
                            createdTypeNameWithoutPatchedPostfix, out var matchingTypeInExistingAssemblies))
                    {
                                     ...
                                     .Where(m => !ExcludeMethodsDefinedOnTypes.Contains(m.DeclaringType)))
                        {
                            ...
                            var matchingMethodInExistingType = allDeclaredMethodsInExistingType.SingleOrDefault(m =>
                                m.FullDescription() ==
        ...
        {
            var onScriptHotReloadFnForType = type.GetMethod(OnHotReloadMethodName,
            ...
            if (onScriptHotReloadFnForType != null)
                UnityMainThreadDispatcher.Instance.Enqueue(() =>
                {
                    ...
                    {
                        LoggerScoped.LogWarning(
                            $"Type: {type.Name} is not {nameof(MonoBehaviour)}, {OnHotReloadMethodName} method can't be executed. You can still use static version: {OnHotReloadNoInstanceStaticMethodName}");
                        return;
                    }

                    foreach (var instanceOfType in
                             GameObject.FindObjectsOfType(type)) //TODO: perf - could find them in different way?
