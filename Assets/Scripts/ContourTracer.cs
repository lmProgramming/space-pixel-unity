...
{
    private readonly List<Stack<Vector2Int>> pixelPaths = new();
    ...
    public int GetPath(int index, ref List<Vector2> path)
    {
        ...
        Vector2 Point()
        {
            return (Vector2)points[pointIndex] * pointMultiplier - pointOffset;
        }

        if (points.Length > path.Count)
        {
            for (pointIndex = 0; pointIndex < path.Count; ++pointIndex) path[pointIndex] = Point();
            for (; pointIndex < points.Length; ++pointIndex) path.Add(Point());
        }
        else
        {
            for (pointIndex = 0; pointIndex < points.Length; ++pointIndex) path[pointIndex] = Point();
    ...
    public void Trace(Texture2D texture, Vector2 pivot, float pixelsPerUnit, uint gapLength, float product)
    {
        ...
        bool IsBorderSafe(int _x, int _y)
        {
            return _y >= 0 && _y < texture.height && _x >= 0 && _x < texture.width && IsBorder(_x, _y);
        }
        ...
        void Encode(Code _code)
        {
            switch (_code)
            {
                ...
                case Code.InnerOuter:
                    if (code != Code.InnerOuter) stack.Push(point);
                    //list.Add(point);
                    break;
                case Code.Straight:
                    if (code != Code.Straight)
